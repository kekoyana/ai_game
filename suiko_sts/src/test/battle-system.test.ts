...
describe('Battle System', () => {
  ...
  it('重装備バフのテスト', () => {
    ...
    store.dispatch({ type: 'gameGeneral/setPlayer', payload: player })
...
store.dispatch(applyHeavyArmor()) // 重装備の効果を適用（ダメージ計算の前に）
state = store.getState()
expect(state.gameGeneral.player.block).toBe(8) // 5 + 3
...
store.dispatch(resetBlock())
store.dispatch(applyHeavyArmor()) // ターン終了時に再度重装備効果を適用
store.dispatch(endTurn())
...
// 2ターン目：重装備の効果が持続
store.dispatch(applyHeavyArmor()) // 重装備の効果を適用（先に）
store.dispatch(takeDamage(14))
store.dispatch(resetBlock())
store.dispatch(applyHeavyArmor()) // ターン終了時に再度重装備効果を適用
store.dispatch(endTurn())
